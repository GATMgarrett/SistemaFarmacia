...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--meses', type=int, default=36, help='Número de meses a simular')
        parser.add_argument('--fecha_inicio', type=str, default=None, help='Fecha de inicio (YYYY-MM-DD)')
        parser.add_argument('--fecha_fin', type=str, default=None, help='Fecha fin (YYYY-MM-DD)')
    ...
    def handle(self, *args, **options):
        ...
        fecha_fin_str = options['fecha_fin']
        seed = options['seed']
        ...
        fecha_fin = date(2025, 6, 2)  # Fecha actual (2 de junio de 2025)
        
        if fecha_inicio_str:
            fecha_inicio = datetime.strptime(fecha_inicio_str, '%Y-%m-%d').date()
        else:
            # Si no se especifica, empezar hace 3 años desde la fecha actual
            fecha_inicio = date(2022, 6, 2)  # 3 años atrás
            ...
            with transaction.atomic():
                try:
                    ...
                    for medicamento in medicamentos_seleccionados:
                        # Generar cantidad y precios (ajustados a la realidad boliviana: 3-100 Bs)
                        cantidad = random.randint(50, 200)
                        precio_compra = decimal.Decimal(random.uniform(3.0, 100.0)).quantize(decimal.Decimal('0.01'))
    ...
    def _simular_ventas_mes(self, fecha_inicio_mes, fake):
        ...
        for dia in range(1, dias_en_mes + 1):
            ...
            # Ajustado para Bolivia: invierno en julio, verano en enero
            es_invierno = fecha_venta.month in [5, 6, 7, 8]  # Mayo a Agosto en Bolivia
            es_verano = fecha_venta.month in [11, 12, 1, 2]  # Noviembre a Febrero en Bolivia
            ...
            if es_invierno:
                num_ventas_base += random.randint(3, 6)  # Más enfermedades respiratorias en invierno
            if es_verano:
                num_ventas_base += random.randint(1, 3)  # Más ventas en verano por otras razones
            ...
            for i in range(num_ventas_base):
                ...
                # Ajustado para que haya más probabilidad de clientes existentes (50% en lugar de 30%)
                if random.random() < 0.5:  # 50% de probabilidad de crear nuevo cliente
                ...
                for medicamento_id in medicamentos_seleccionados:
                    ...
                    # Verificar si hay suficiente stock y si no, realizar reabastecimiento
                    stock_disponible = sum(lote.cantidad for lote in lotes_med)
                    if stock_disponible < cantidad_vender:
                        # Si el stock es bajo, podríamos realizar una compra de emergencia
                        if stock_disponible <= 5 and random.random() < 0.8:  # 80% de probabilidad de reabastecimiento
                            try:
                                # Seleccionar un proveedor aleatorio
                                proveedores = list(Proveedores.objects.filter(activo=True))
                                if proveedores:
                                    ...
                                    # Compra de emergencia solo de este medicamento
                                    cantidad_compra = random.randint(50, 150)
                                    precio_compra = decimal.Decimal(random.uniform(3.0, 100.0)).quantize(decimal.Decimal('0.01'))
                                    precio_venta = precio_compra * decimal.Decimal(random.uniform(1.2, 1.5)).quantize(decimal.Decimal('0.01'))
                                    
                                    # Crear compra de emergencia
                                    with transaction.atomic():
                                        compra = Compras.objects.create(
                                            proveedor=proveedor,
                                            fecha_compra=fecha_venta,
                                            precio_total=precio_compra * decimal.Decimal(cantidad_compra),
                                            activo=True
                                        )
                                        ...
                                        lote_nuevo = LoteMedicamento.objects.create(
                                            ...
                                            precio_venta=precio_venta,
                                            fecha_compra=fecha_venta,
                                            fecha_produccion=fecha_venta - timedelta(days=random.randint(30, 90)),
                                            fecha_vencimiento=fecha_venta + timedelta(days=random.randint(180, 720)),
                                            lote_fabricante=f'LOT-{fake.bothify(text="??###")}',
                                            activo=True
                                        )
                                        ...
                                        lotes_med.append(lote_nuevo)
                                        stock_disponible += cantidad_compra
                                        
                                        self.stdout.write(f'  Reabastecimiento de emergencia para {medicamento.nombre}: {cantidad_compra} unidades')
                            except Exception as e:
                                logger.error(f'Error al crear compra de emergencia: {str(e)}')
                        
                        # Ajustar cantidad a vender según stock disponible (incluso después del reabastecimiento)
                        if stock_disponible < cantidad_vender:
